...
let usuarioTipo = "admins"; // Tipo de usuário adicionado (admin, aluno, admin)
...
Given('Eu estou na página {string}', (pagina) => {
  ...
  When('Eu adiciono o admin {string} de login {string} e senha {string} no sistema', (nome, login, senha) => {
    usuarioLogin = login;
...
// New step to verify the admin exists in the database
And('o usuario do tipo {string} de login {string} está no banco de dados', (tipoUsuario, login) => {
  ...
  // Passo para verificar se o usuário admin com login específico existe
And('existe o usuário admin com login {string}', (login) => {
  cy.get('.usuarios-list')
    .should('contain', login);
});

  // Passo para verificar se o usuário admin com login específico não existe
And('não existe o usuário admin com login {string}', (login) => {
  cy.get('.usuarios-list')
